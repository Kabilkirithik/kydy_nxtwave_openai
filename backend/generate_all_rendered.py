"""
Script to generate rendered HTML output files for all existing lessons and sessions.
"""
import json
import sys
...
def generate_rendered_html(lesson: dict, api_base: str = "") -> str:
        ...
        function animateStep(stepIndex) {
            ...
            svgs.forEach((svg, svgIdx) => {
                const timer = setTimeout(() => {
                    ...
                    svg.querySelectorAll('path').forEach((path, pIdx) => {
                    ...
                    svg.querySelectorAll('line').forEach((line, lIdx) => {
                    ...
                    svg.querySelectorAll('rect').forEach((rect, rIdx) => {
                    ...
                    svg.querySelectorAll('text').forEach((text, tIdx) => {
                    ...
                    svg.querySelectorAll('circle').forEach((circle, cIdx) => {
...
def generate_for_lesson(lesson_id: str):
    """Generate rendered HTML for a lesson."""
    lesson_file = DATA_DIR / f"lesson_{lesson_id}.json"
    if not lesson_file.exists():
        print(f"  ⚠️  Lesson file not found: {lesson_file}")
        return False
    ...
    try:
        ...
        rendered_file = RENDERED_DIR / f"lesson_{lesson_id}.html"
        
        with open(rendered_file, "w", encoding="utf-8") as f:
            f.write(rendered_html)
        
        print(f"  ✅ Generated: {rendered_file.name}")
        return True
    except Exception as e:
        print(f"  ❌ Error: {e}")
        return False

def generate_for_session(session_id: str):
    """Generate rendered HTML for a session."""
    ...
    try:
        ...
        if not lesson_id:
            print(f"  ⚠️  Session has no lesson_id")
            return False
        
        lesson_file = DATA_DIR / f"lesson_{lesson_id}.json"
        if not lesson_file.exists():
            print(f"  ⚠️  Lesson {lesson_id} not found for session")
            return False
        ...
        rendered_html = generate_rendered_html(lesson, "")
        rendered_file = RENDERED_DIR / f"session_{session_id}.html"
...
def main():
    """Generate rendered outputs for all lessons and sessions."""
    RENDERED_DIR.mkdir(exist_ok=True)
    
    print("🎬 Generating rendered HTML outputs...\n")
    
    lesson_files = list(DATA_DIR.glob("lesson_*.json"))
    print(f"📚 Found {len(lesson_files)} lesson(s)")
    lesson_count = 0
    for lesson_file in lesson_files:
        lesson_id = lesson_file.stem.replace("lesson_", "")
        print(f"\n📖 Processing lesson: {lesson_id}")
        if generate_for_lesson(lesson_id):
            lesson_count += 1
    ...
    for session_file in session_files:
        ...
        if generate_for_session(session_id):
            session_count += 1
    
    print(f"\n\n✨ Summary:")
    print(f"   Lessons: {lesson_count}/{len(lesson_files)} generated")
    print(f"   Sessions: {session_count}/{len(session_files)} generated")
    print(f"   Total files in {RENDERED_DIR}: {len(list(RENDERED_DIR.glob('*.html')))}")
